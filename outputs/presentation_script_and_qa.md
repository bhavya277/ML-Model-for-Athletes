...
- **Key Takeaway:** 3,000 training athletes and 1,100 test athletes spanning 6 sports, synthesized across 5 active data streams with zero demographic drift.
- **Presenter Script:**
  > "Our dataset spans 3,000 training athletes and 1,100 test athletes with zero athlete overlap. We ingest five distinct data modalities: static metadata, daily activity logs, sleep day logs, training session records, and continuous hourly heart rate. Our forensic audit verified that demographic features between train and test cohorts exhibit zero covariate shift via Kolmogorov-Smirnov tests with p-values exceeding 0.18."
- **Likely Judge Question:**
  > *"Why did you exclude the weight log and hourly steps/calories files?"*
- **Recommended Answer:**
  > *"We conducted controlled ablation experiments. Weight logs were missing for 33% of athletes and provided no additional signal beyond baseline weight in metadata. Hourly steps and calories used non-ISO timestamps that duplicated the daily activity merged table. Excluding them eliminated noise, accelerated feature generation, and ensured temporal safety."*
...
- **Key Takeaway:** Acute-to-chronic workload spikes ($ACWR > 1.30$) provide strong predictive signal for early injury onset ($r = -0.863$); chronic sleep debt is associated with higher injury rates.
- **Presenter Script:**
...
- **Key Takeaway:** Task A ROC-AUC = 0.7624 (F1 = 0.6621, Precision = 97.23%); Conditional Onset MAE = 2.64d ($R^2 = 0.782$); Conditional Recovery MAE = 2.96d ($R^2 = 0.204$).
- **Presenter Script:**
  > "Here are our final out-of-fold validation metrics: For Task A, our classifier achieves a ROC-AUC of 0.7624 (Fold mean: 0.7627 ± 0.017) and an F1-score of 0.6621 at threshold 0.50, delivering 97.23% precision. For Task B among identified injured athletes, onset timing is predicted with an MAE of 2.64 days (R² = 0.7820) and recovery duration with an MAE of 2.96 days (R² = 0.2037). Under the official penalized metric with n_risk=30 on false negatives, penalized onset MAE is 15.31 days and penalized recovery MAE is 16.42 days."
- **Likely Judge Question:**
