...
export const RiskTrendChart = ({ telemetry = [], baselineSteps = 8000, baselineSleep = 480 }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-surface-100 border border-border-default p-3 rounded-lg shadow-xl text-xs space-y-1.5 min-w-[190px]">
  ...
  return (
    <Card className="border border-border-default bg-surface-200/80">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
        <div>
          <h3 className="text-base font-semibold text-slate-100 tracking-tight">
            Historical Telemetry & Workload Dynamics
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Daily multi-window tracking against 30-day baseline reference
          </p>
        </div>
            ...
            {[7, 14, 30].map(days => (
              <button
                ...
                className={`px-2 py-1 rounded-md text-xs font-medium transition-all ${
                  windowDays === days ? 'bg-surface-50 text-brand-400 font-bold' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
                {days}d
              </button>
      ...
      <CardBody className="h-60 sm:h-72">
        <ResponsiveContainer width="100%" height="100%">
          {metricMode === 'steps' ? (
            <AreaChart data={filteredData} margin={{ top: 10, right: 10, bottom: 0, left: -10 }}>
              <defs>
              ...
              <ReferenceLine y={baselineSteps} stroke="#64748b" strokeDasharray="4 4" label={{ value: 'Baseline', fill: '#94a3b8', fontSize: 8, position: 'insideTopLeft' }} />
              <ReferenceLine y={spikeThresholdSteps} stroke="#f43f5e" strokeDasharray="3 3" label={{ value: 'Spike (1.3x)', fill: '#f43f5e', fontSize: 8, position: 'insideTopRight' }} />
            ...
            <AreaChart data={filteredData} margin={{ top: 10, right: 10, bottom: 0, left: -10 }}>
              ...
              <ReferenceLine y={baselineSleep} stroke="#64748b" strokeDasharray="4 4" label={{ value: '8h Target', fill: '#94a3b8', fontSize: 8, position: 'insideTopLeft' }} />
              <RechartsTooltip content={<CustomTooltip />} />
      ...
      <div className="pt-3 border-t border-border-subtle/50 flex flex-wrap items-center justify-between text-xs text-slate-400 gap-1.5">
        <span className="flex items-center space-x-1.5 text-[11px]">
          <span className="w-2 h-2 rounded-full bg-brand-400 flex-shrink-0" />
          <span>Observed historical telemetry (2026-01-05 &rarr; 2026-02-03)</span>
        </span>
